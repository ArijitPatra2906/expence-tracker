...
function AddEditTransaction({
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (edit) {
        await axios.post(
          `${process.env.REACT_APP_BASEURL}/api/transaction/edit-transaction`,
          { payload: { ...values, userId: user._id }, transactionId: edit._id }
        );
        getTransaction();
        message.success("Transaction Updated Successfull");
      } else {
        await axios.post(
          `${process.env.REACT_APP_BASEURL}/api/transaction/add-transaction`,
