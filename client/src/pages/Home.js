...
function Home() {
  ...
  const getTransaction = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_BASEURL}/api/transaction/get-alltransaction`,
        {
          userId: user._id,
          frequency,
  ...
  const deleteTransaction = async (record) => {
    ...
    try {
      setLoading(true);
      await axios.post(
        `${process.env.REACT_APP_BASEURL}/api/transaction/delete-transaction`,
  ...
  const columns = [
    {
      ...
      key: "name",
      render: (record) => (
        <label className="moment">{record.substring(0, 10)}</label>
      ),
    },
    {
      title: "Amount",
        ...
        <div className="d-flex">
          ...
          <div className="d-flex flex-column mx-5">
            ...
            <Select value={type} onChange={(value) => setType(value)}>
              ...
              <Select.Option value="alltransaction">
                All Transaction
              </Select.Option>
            </Select>
          </div>
